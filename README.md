...
This also improves search performance by using an **O(w * (log(n) + m) + m * log(m))** index design and lookup algorithm, **n** being the total amount of unique terms, **m** being the number of URLs chosen to be outputted (generally being a low number, 10 by default), and **w** being the amount of words in the query.
