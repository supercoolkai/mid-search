...
This also improves search performance by using an **O(w * (log(n) + m) + u * log(u))** index design and lookup algorithm,
**n** being the total amount of unique terms, **m** being the number of URLs chosen to be outputted,
**u** being the number of unique URLs ranked, and **w** being the amount of words in the query.
